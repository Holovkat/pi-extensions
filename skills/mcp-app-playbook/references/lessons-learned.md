...
- define the data contract early so lane/app boundaries stay compact and predictable
...
- dumping too much raw state across app/lane boundaries instead of using narrow query or summary paths

## Strong Recommendations

1. Start with a written session model.
2. Choose single-endpoint vs multi-endpoint early.
3. Write the data contract before implementation: input scope, output scope, ownership, mutation authority, and transfer limits.
4. Build external widget compatibility up front.
5. Restart and verify the actual runtime after each hosting-layer change.
6. Keep a standard smoke-test script with the app from day one.
