...
class _ClockState extends State<_Clock> { Timer? timer; @override void initState() { super.initState(); timer = Timer.periodic(const Duration(seconds: 1), (_) { if (mounted) setState(() {}); }); } @override void dispose() { timer?.cancel(); super.dispose(); } @override Widget build(BuildContext context) => Center(child: Column(mainAxisSize: MainAxisSize.min, children: [Text(TimeOfDay.now().format(context), style: Theme.of(context).textTheme.displaySmall?.copyWith(fontSize: (Theme.of(context).textTheme.displaySmall?.fontSize ?? 36) - 2)), const SizedBox(height: 8), Text(widget.workspace, textAlign: TextAlign.center, style: Theme.of(context).textTheme.bodySmall)])); }
